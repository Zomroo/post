...
def handle_callback(client, callback_query):
    ...
    if action == 'confirm':
        ...
        if message.photo:
            ...
            client.copy_message(chat_id=channel_id, from_chat_id=message.chat.id, message_id=message.id, text=caption, reply_markup=keyboard)
        else:
            ...
            buttons = [InlineKeyboardButton(text=f"Link {i+1}", url=link) for i, link in enumerate(links)]
            keyboard = InlineKeyboardMarkup([buttons])
            client.send_message(chat_id=channel_id, caption=caption, reply_markup=keyboard)
