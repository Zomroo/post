...
def handle_message(client, message):
    ...
    if message.text:
        ...
        if message.text.startswith('http'):
            ...
            # Ask for confirmation with the title in the caption
            confirmation_message = f"Are you sure you want to send this message?\nTitle: {title}"
            confirm_button = InlineKeyboardButton(text="Confirm", callback_data=f"confirm_{message.id}")
            cancel_button = InlineKeyboardButton(text="Cancel", callback_data=f"cancel_{message.id}")
            keyboard = InlineKeyboardMarkup([[confirm_button, cancel_button]])
    ...
    if message.caption:
        ...
        title = lines[0].strip()  # Extract the title from the first line of the caption
...
def handle_callback(client, callback_query):
    ...
    if action == 'confirm':
        ...
        if message.photo:
            ...
            keyboard = InlineKeyboardMarkup(buttons[i:i+1] for i in range(0, len(buttons), 1))
